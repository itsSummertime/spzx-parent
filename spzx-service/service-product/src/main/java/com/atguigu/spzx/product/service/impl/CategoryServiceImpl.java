...
public class CategoryServiceImpl  implements CategoryService {
    ...
    @Cacheable(value = "category", key ="'findFirstCategory'")
    @Override
    ...
    public List<Category> findCategoryTree() {
        ...
        firstList.forEach(first->{
            ...
            List<Category> secondList = allList.stream().filter(category -> category.getParentId().equals(firstId)).toList();
            //添加二级分类到一级分类中
            ...
            secondList.forEach(second->{
                ...
                List<Category> thirdList = allList.stream().filter(category -> category.getParentId().equals(secondId)).toList();
                //添加三级分类到二级分类中
